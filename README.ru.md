# <img src="docs/icons/brain.svg" width="28" alt="" /> Crystallized v2.0
...
## <img src="docs/icons/alert.svg" width="22" alt="" /> Знакомая боль: «День сурка» у нейросетей
...
## <img src="docs/icons/workflow.svg" width="22" alt="" /> Как это работает (простыми словами)

У твоего ИИ-ассистента появляется структурированная память, которая работает как долговременная память у человека:
...
## <img src="docs/icons/flash.svg" width="22" alt="" /> Быстрый старт за 1 минуту
...
## <img src="docs/icons/key.svg" width="22" alt="" /> Вход через Claude Pro/Max (без API-ключей и переплат)
...
> **Безопасность**: твои токены и пароли никуда не отправляются. Всё происходит строго на твоем компьютере.  
> Подробная инструкция: [docs/AUTHENTICATION.md](docs/AUTHENTICATION.md).

---

## <img src="docs/icons/folder.svg" width="22" alt="" /> Где всё лежит и как устроено
...
## <img src="docs/icons/tools.svg" width="22" alt="" /> Что умеет твой агент (MCP-инструменты)

Когда Crystallized подключен, агент получает набор инструментов:
...
## <img src="docs/icons/tools.svg" width="22" alt="" /> Каталог MCP: дать агенту руки

Память — это то, что агент знает. Отдельно от неё можно подключить то, что агент **умеет**: навигацию по коду на уровне символов, граф зависимостей репозитория, живой браузер, Android-устройство, поисковую аналитику и монтаж видео.

Готовые шаблоны конфигурации лежат в [`config/mcp/`](config/mcp/), подробное описание каждого сервера, требований и порядка слияния с `opencode.json` — в **[docs/MCP_CATALOG.md](docs/MCP_CATALOG.md)**.

| Сервер | Что даёт агенту |
| --- | --- |
| `serena` | Поиск определений, ссылок и правка кода по символам, а не по строкам |
| `codebase-memory` | Граф вызовов и импортов, анализ влияния правки до её внесения |
| `playwright` | Реальный браузер: клики, формы, скриншоты, сеть и консоль |
| `design-cockpit` | Дизайн-брифы по существующим компонентам и аудит доступности |
| `flutter-dev` | Сборка, установка, hot reload и скриншоты Android-приложения |
| `seo-analytics` | Google Search Console, Яндекс Вебмастер и Метрика |
| `fcp` | Автоматизация Final Cut Pro через FCPXML и анализ медиафайлов |

Всё модульно: ставится по одному, включается и выключается флагом `"enabled"`. Ни один из этих серверов не нужен для работы самой памяти.

> **Безопасность**: в шаблонах только заглушки. Настоящие токены держи в `~/.config/opencode/secrets/` и подключай через `{file:~/.config/opencode/secrets/your-token}` — так ключ не попадёт в git.

---

## <img src="docs/icons/laptop.svg" width="22" alt="" /> Переезд на новый ноутбук за 10 секунд
